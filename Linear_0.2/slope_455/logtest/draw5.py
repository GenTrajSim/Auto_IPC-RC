...
#x = x[sample_indices]
#y = y[sample_indices]
x_edges=np.linspace(-225.97,155.97,200)
y_edges=np.linspace(-2,2,200)
H, x_edges, y_edges = np.histogram2d(x,y, bins=[x_edges,y_edges])
H_prob = H/np.sum(H)
H_prob = -1*np.log(H_prob) #- H_prob.min()
plt.figure(figsize=(6,6))
print(H_prob.min())
#plt.hist2d(y,x,bins=100,cmap='jet',range=[[-4,5],[-2,2]])
plt.imshow(H_prob.T,vmin = 6 ,  origin='lower', extent=[-225.97,155.97,-2,2],aspect='auto',cmap='jet')
