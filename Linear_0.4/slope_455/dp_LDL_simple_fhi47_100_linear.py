...
path = glob.glob("../../../dp_LDL/Dataset/*/data*/new_coord/*npy") + glob.glob("../../../dp_LDL/BalanceDataset/*/data*/new_coord/*npy")
#+glob.glob("/kaggle/input/newldldata/data2/data2/new_coord/*npy")
...
class LDL_Data_Feeder:
    ...
    def read_npy_file(self, filename1,filename2):
    ...
    def Generator_Dataset(self):
        ...
        for i in range(len(idn)):
            ...
            out = self.read_npy_file(coord_path,sysin_path)
            #tf.print(out[0][0].shape,',', out[1][0].shape,',', out[2][0].shape,',', out[3][0].shape,',', out[4][0].shape)
            yield out[0][0], out[1][0], out[2][0], out[3][0], out[4][0], out[5][0], out[6][0]#,  out[7][0]#, out[8][0]
...
#    tf.print('correlation.shape',correlation.shape)
    ...
    #
    loss_correlation = (tf.reduce_mean((correlation-0.4)**2) )
    loss_spearman_cor = (tf.reduce_mean((spearman_cor-0.4)**2) )
...
#    tf.print('k_cor',k_cor.shape)
    #
    aa_loss1 = tf.reduce_mean( (k_cor - tf.math.tan(455.*pi/1000.0))**2 )  #aa_loss1 + aa_loss2
...
optimizer1 = tf.keras.optimizers.Adam(0.00008) #0.0001
optimizer2 = tf.keras.optimizers.Adam(0.00008) #0.0001
@tf.function
...
@tf.function
def test_step(coord,rho,pot,temp,press,epoch):
    filename = './log_simple_fhi47_100_linear/test.log'
    fle = open(filename,'a')
    Prho,x3,x2,x1, Ppot,y3,y2,y1 = model_encoder(coord, temp, press, training=True)
    x3,y3,x4, normal_x3,normal_y3 = model_decoder(x3,x2,x1, y3,y2,y1, temp,press,training=True)
    loss1, loss2, loss_rho, loss_pot, aa_loss,linear_aax4,loss_correlation = loss_function_1(Prho,rho, Ppot,pot, normal_y3,x4,x3)
    ...
    filename1 = './log_simple_fhi47_100_linear/xe.log'
    flx = open(filename1,'w')
    tf.print("real_rho:", rho, ", pred_rho:", Prho,  output_stream = 'file://'+flx.name)
    tf.print("===================================", output_stream = 'file://'+flx.name)
    tf.print("real_pot:", pot, ", pred_pot:", Ppot, output_stream = 'file://'+flx.name)
    out = None
    out = tf.concat([x3, x4],axis=-1)
    out = tf.concat([normal_y3, out],axis=-1)
    tf.print(out, summarize=300,output_stream = 'file://'+flx.name)
    #tf.print("===================================", output_stream = 'file://'+flx.name)
...
for epoch in range(2000000):
    ...
    data_Feeder = LDL_Data_Feeder(
        path, max_batch_system = 20000, local_atom_maxN = 31, system_OatomN = 300
    )
    batch_dataset = data_Feeder.Generate_batch(batch_size=300,Repeat_size=2, shuffle_size=400)
    batch_dataset = batch_dataset.cache()
    ...
    if epoch % 100 == 0:
        ...
        for (batch,(coord,rho,pot,ent,pb,temp,press)) in enumerate(test_dataset):
            test_step(coord,rho,pot,temp,press,epoch)
